...
require_once('util/metrics/WPSDPinterest.php');
...
height:17px;
	...
	if($opts == null || $opts['age']){
		...
		if($siteage->domainOK()) { 
			...
			wpsd_helper_add_row('age', 	'<span>Site age:</span>', 
			__('Not available for your domain', 'wpsd'), 2);
		}
	}
	...
	if($opts == null || $opts['delicious']){
		...
		wpsd_helper_add_row('ds',
			...
			sprintf('<a href="%s" title="Delicious | Url Result Count" target="_blank" rel="nofollow">%s</a>', $ds->getAddress(), $ds->getUrlResultCount() ), 1 );
	...
	if($opts == null || $opts['stumbleupon']){
		...
		wpsd_helper_add_row('su', 
			...
			sprintf('<a href="%s" title="StumbleUpon | Likes" target="_blank" rel="nofollow">%s</a>', $su->getProfileAddress(), $su->getLikes()), 2 );
	...
	if($opts == null || $opts['pinterest']){	
		
		$pint = new WPSDPinterest();
	
		wpsd_helper_add_row('pinterest', '<span>Pinterest:</span>', 
			sprintf('<a href="%s" target="_blank" title="Pinterest | Pins" rel="nofollow">%s</a>', 
			$pint->getAddress(), $pint->getPins()) . ' | ' . 
			sprintf('<a href="%s" target="_blank" title="Pinterest | Boards" rel="nofollow">%s</a>', 
			$pint->getAddress(), $pint->getBoards()) . ' | ' . 
			sprintf('<a href="%s" target="_blank" title="Pinterest | Followers" rel="nofollow">%s</a>', 
			$pint->getAddress(), $pint->getFollowers()). ' | ' . 
			sprintf('<a href="%s" target="_blank" title="Pinterest | Following" rel="nofollow">%s</a>', 
			$pint->getAddress(), $pint->getFollowing()) . ' | ' . 
			sprintf('<a href="%s" target="_blank" title="Pinterest | Likes" rel="nofollow">%s</a>', 
			$pint->getAddress(), $pint->getLikes()), 2);
