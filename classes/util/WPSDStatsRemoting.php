...
class WPSDStatsRemoting {
	 ...
	 * @return void
	 */
	function handleData($result, $key, $type) {
	
		$pattern = '<div id="'.$type.'" class="statsdiv">(.*?)<\/div>';
	...
	function splitData($result, $key) {
		...
		$this->handleData($result, $key, 'referrers');
		
		$this->handleData($result, $key, 'postviews');
		
		$this->handleData($result, $key, 'searchterms');
		
		$this->handleData($result, $key, 'clicks');
		...
		if(!empty($key) && is_array($matches)) {
			...
			if(isset($matches[1]) && isset($matches[2])) {
				...
				for($i=0; $i < count($matches[1]); $i++) {
					...
					$val1 = $this->cleanStr(trim($matches[1][$i]));
					$val2 = $this->cleanStr(trim($matches[2][$i]));
	...
	function cleanStr($str) {
		$str = utf8_decode($str);
		return htmlentities(strip_tags(str_replace(']', ')', str_replace('[', '(', $str))) );
	}		
}
?>