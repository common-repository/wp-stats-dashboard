...
define('WPSD_UA', 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.1 (KHTML, like Gecko) Chrome/22.0.1207.1 Safari/537.1');
...
function wpsd_get_login_postdata() {
	...
	return "log={$un}&pwd={$pw}&testcookie=1&wp-submit=Log In&redirect_to=wp-admin/index.php?page=stats&blog={$blogId}";
}
...
function wpsd_get_login_url() {
	...
	return 'https://dashboard.wordpress.com/wp-login.php';
}
...
function wpsd_get_all_time_views() {
	...
	preg_match('@<span>([0-9]+)</span> .+@', str_replace(',', '', $result), $matches);
